...
class ChatbotController extends Controller
{
    ...
    public function ask(Request $request)
    {
        if (! Schema::hasTable('chatbot_messages')) {
            return back()
                ->with('error', 'Chưa có bảng chatbot_messages. Vui lòng chạy migration trước.')
        ...
        return back()
            ->with('success', 'Chatbot đã phản hồi câu hỏi của bạn.')
            ->with('chatbot_open', true);
    }
    ...
    private function answer(string $question): string
    {
        ...
        return match (true) {
            ...
            str_contains($text, 'lịch kiểm tra') || str_contains($text, 'lịch thi') => 'Bạn có thể mở mục Lịch kiểm tra để xem môn học, ngày kiểm tra, phòng và ghi chú liên quan.',
            ...
            default => 'Tôi có thể hỗ trợ tra cứu nhanh về điểm, thời khóa biểu, lịch kiểm tra, điểm danh, tài liệu học tập và thông báo của nhà trường.',
        };
    }
}
